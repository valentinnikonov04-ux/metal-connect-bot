...
window.addEventListener("error", function (event) {
  ...
  errorBox.textContent = "Ошибка сайта: " + (event.message || "неизвестная ошибка");
...
var state = {
  role: "",
  user: siteUser(),
  authToken: safeGet("mc_site_token", ""),
  authMode: "login",
...
function siteUser() {
  var saved = {};
  try {
    saved = JSON.parse(safeGet("mc_site_user", "{}")) || {};
  } catch (error) {
    saved = {};
  }
  return {
    id: saved.id || "",
    first_name: saved.full_name || saved.first_name || "",
    username: saved.username || saved.email || saved.phone || "",
...
function resolveApiUrl() {
  var defaultUrl = normalizeApiUrl(window.DEFAULT_API_URL || "");
  var savedUrl = normalizeApiUrl(safeGet("mc_site_api_url", ""));
  var isLocalPage = window.location.hostname === "127.0.0.1" || window.location.hostname === "localhost";
  var localDefaultUrl = isLocalPage ? "http://127.0.0.1:8092" : "";
  var raw = params.get("api") || localDefaultUrl || defaultUrl || savedUrl || "";
  var normalized = normalizeApiUrl(raw);
  if (normalized) safeSet("mc_site_api_url", normalized);
  return normalized;
}
...
function setApiUrl(value) {
  ...
  if (API_BASE) {
    safeSet("mc_site_api_url", API_BASE);
    updateApiTools();
    loadData();
    return true;
  }
...
function saveAuth(data) {
  ...
  state.user = {
    ...
    first_name: data.user && (data.user.full_name || data.user.company || data.user.email) || "",
    username: data.user && (data.user.username || data.user.email || data.user.phone) || "",
    photo_url: ""
  };
  safeSet("mc_site_token", state.authToken);
  safeSet("mc_site_user", JSON.stringify(data.user || {}));
  safeSet("mc_site_role", state.role || "");
}
...
function updateAuthMode(mode) {
  ...
  $("[data-auth-mode='login']").classList.toggle("active", state.authMode === "login");
  $("[data-auth-mode='register']").classList.toggle("active", state.authMode === "register");
  $("#authTitle").textContent = state.authMode === "register" ? "Регистрация" : "Вход";
  $("#authSubmit").textContent = state.authMode === "register" ? "Создать аккаунт" : "Войти";
  $("#authHint").textContent = state.authMode === "register"
    ? "Укажите email или телефон и пароль от 6 символов."
...
async function submitAuth(event) {
  ...
  if (!payload.email && !payload.phone) {
    setFormMessage("authMessage", "Укажите email или телефон.", "error");
    return;
  }
  if (!payload.password || payload.password.length < 6) {
    setFormMessage("authMessage", "Пароль должен быть не короче 6 символов.", "error");
    return;
  }
  try {
    setFormMessage("authMessage", "");
    setButtonLoading(button, true, state.authMode === "register" ? "Создаю..." : "Вхожу...");
    var data = await authPost(state.authMode === "register" ? "/api/register" : "/api/login", payload);
    saveAuth(data);
    if (!state.role) {
    ...
    setFormMessage("authMessage", readableApiError(error.message), "error");
  } finally {
...
async function assignDefaultRole() {
  state.role = "customer";
  safeSet("mc_site_role", state.role);
  await apiPost("/api/profile/update", { role: state.role });
}
...
async function chooseRole(role) {
  ...
  safeSet("mc_site_role", state.role);
  try {
    setFormMessage("roleMessage", "");
    await apiPost("/api/profile/update", { role: state.role });
    showAppScreen();
    await loadData();
  } catch (error) {
    setFormMessage("roleMessage", "Не удалось сохранить роль: " + readableApiError(error.message), "error");
  }
}

async function logout() {
  try {
    if (state.authToken) await apiPost("/api/logout", {});
  } catch (error) {
  }
  state.authToken = "";
  state.role = "";
  safeSet("mc_site_token", "");
  safeSet("mc_site_user", "{}");
  safeSet("mc_site_role", "");
  state.user = siteUser();
...
async function bootstrapAuth() {
  ...
  try {
    ...
    saveAuth({ token: state.authToken, user: me.user || {}, role: me.role || safeGet("mc_site_role", "") });
    if (!state.role) {
      await assignDefaultRole();
    }
    showAppScreen();
...
function readableApiError(message) {
  ...
  if (text.indexOf("User with this email already exists") !== -1) return "Пользователь с таким email уже зарегистрирован. Переключитесь на вход.";
  if (text.indexOf("User with this phone already exists") !== -1) return "Пользователь с таким телефоном уже зарегистрирован. Переключитесь на вход.";
  if (text.indexOf("Invalid login or password") !== -1) return "Неверный логин или пароль.";
  if (text.indexOf("Password must contain") !== -1) return "Пароль должен быть не короче 6 символов.";
  if (text.indexOf("Failed to fetch") !== -1 || text.indexOf("Load failed") !== -1) return "API недоступен. Проверьте, что api_site.py запущен и URL указан верно.";
  return text.replace(/^API \\d+: /, "") || "Неизвестная ошибка.";
}
...
async function publishOrder() {
  ...
  try {
    ...
    await apiPost("/api/orders/create", payload);
...
function initEvents() {
  ...
  $$("[data-auth-mode]").forEach(function (button) {
    button.addEventListener("click", function () { updateAuthMode(button.dataset.authMode); });
  });
  $$("[data-role-choice]").forEach(function (button) {
    button.addEventListener("click", function () { chooseRole(button.dataset.roleChoice); });
  });
...
function orderFileUrl(order) {
  ...
  var fileId = order.file_id || order.photo_id || order.file_preview || "";
  if (order.file_url) {
    if (String(order.file_url).indexOf("data:") === 0) return order.file_url;
    if (/^https?:\/\//.test(order.file_url)) return order.file_url;
    return API_BASE + order.file_url;
  }
...
function openOrderFile(orderId) {
  ...
  if (!url) {
    showToast("Файл недоступен в веб-версии.");
    return;
  }
...
function settingStorageKey(name) {
  return "mc_setting_" + (state.user.id || "anon") + "_" + name;
}
